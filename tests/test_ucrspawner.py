...
def test_get_constraints(monkeypatch):
    ...
    spawner.marathon_constraints = [['foo', 'LIKE', 'bar'], ['bar', 'UNLIKE', 'foo'], ['a', 'IS', 'b']]
    assert spawner.get_constraints() == [MarathonConstraint('foo', 'LIKE', 'bar'), MarathonConstraint('bar', 'UNLIKE', 'foo'), MarathonConstraint('a', 'IS', 'b')]
